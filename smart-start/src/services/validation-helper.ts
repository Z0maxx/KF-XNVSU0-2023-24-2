...
export function isRequired(validations?: Array<string>) {
    if (!validations) return false

    return validations.some(v => v.includes('required'))
}

export function setRequiredFields(requireds: Requireds, validations: Validations | undefined) {
...
export function setErrors(errors: Errors, validations: Validations) {
...
export function resetErrors(errors: Errors) {
    Object.keys(errors).forEach(field => {
        errors[field].value = []
    })
}
