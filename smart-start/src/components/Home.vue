<template>
  <div>
    ...
    <p class="md:text-2xl">Let people rate and comment on your ideas before spending money on your startup ideas!</p>
    <div class="mt-4 text-5xl"><span class="count">{{ ideaCount }}</span> Ideas with <span class="count">{{ ratingCount }}</span> Ratings and <span class="count">{{ commentCount }}</span> Comments</div>
    <div class="mt-4 flex flex-wrap sm:flex-nowrap bg-green-200 rounded-lg p-2">
...
onMounted(() => {
  ...
  if (icons.value) {
    for (let i = 0; i < (window.innerWidth * window.innerHeight) / 35000; i++) {
      const svg = svgs[getRandomInt(0, svgs.length - 1)]
      const size = getRandomInt(20, (window.innerWidth * window.innerHeight) / 20000)
      let positionX = getRandomInt(size, window.innerWidth - size)
      let positionY = getRandomInt(size, window.innerHeight - size)
      icons.value.innerHTML += `
        <div
          class="text-emerald-800"
          style="width: ${size}px; height: ${size}px; position: absolute; top: ${positionY}px; left: ${positionX}px; z-index: -1">
          ${svg}
        </div>`
    }
  }
